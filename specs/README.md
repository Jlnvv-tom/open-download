...
5. **尽量遵循SOLID五大原则** 
单一职责 (Single Responsibility, S)：一个类或一个模块只做一件事，只有一个改动它的理由。
开闭原则 (Open/Closed, O)：对扩展开放，对修改关闭。增加新功能时，不改动旧代码。
里氏替换 (Liskov Substitution, L)：子类可以替换父类，并且程序不会出错。
接口隔离 (Interface Segregation, I)：使用多个专门的接口，不要用一个庞大臃肿的接口。
依赖倒置 (Dependency Inversion, D)：要依赖抽象接口，不要依赖具体的实现