...
| 2026-09-25 | 弹窗按设计稿二次优化 + 单次批量下载上限 500 | 头部加副标题/开关文字/设置齿轮（`openOptionsPage`）；指标条容器化并带图标、媒体 Tab 带图标；卡片信息条改格式徽标 + 大小右对齐（尺寸维度移入 title）；页脚加「已选择 N 项 · 合计大小」汇总与「下载选中」下拉菜单（自动/打包/直下，内存态，`downloadMediaAsZip` 透传 strategy）；`MAX_BATCH_DOWNLOAD = 500`（`handleDownloadZip` 截断并返回 `capped/cappedFrom`，结果提示「已按上限取前 N 条」）；i18n key 153 → 161；测试数 235 → 237 |
