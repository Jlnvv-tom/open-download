...
- 🌐 **双通道捕获** — 网络请求观察 + 页面元素扫描（缓存命中未发请求的图片、视频也能入库），按 URL 去重合并
- 🔍 **智能识别** — 通过 `webRequest` + `Content-Type` + 文件扩展名三重判断，视频条目带时长与封面
- 📦 **ZIP 批量下载** — 打包在后台 offscreen document 中进行，关闭弹窗不中断，进度实时显示
- 🗂️ **大文件安全** — 单文件 >50MB 或整包 >500MB 自动改逐条直下，其余按卷拆分 ZIP，避免内存溢出
- 🎯 **灵活过滤** — 按图片/视频、格式、域名、扩展名、文件大小、图片宽高、来源（网络/DOM）筛选
- 🖼️ **双视图预览** — Popup 支持列表视图和卡片视图，可切换「按页面分组」折叠
- 🚫 **站点级开关** — 弹窗或页面右键菜单即可暂停某个站点的捕获
- ⬇️ **滚动抓取** — 一键自动滚动长列表页，懒加载资源随滚动入库
- 🌏 **中英双语** — 跟随浏览器语言，基于 `chrome.i18n` 的 `_locales`
- ⚙️ **丰富的设置** — 自动下载、保存目录、命名方式、捕获媒体类型、打包携带 Cookie 等
- 📊 **统计面板** — 捕获数 / 已下载 / 失败数
